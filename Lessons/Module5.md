...
bwa-mem2 mem \
    ...
    -o aligned/sample_aligned.bam

# Recommended — add read groups and pipe directly into SAMtools sort
# This avoids writing a large intermediate SAM file to disk
bwa-mem2 mem \
    -t 8 \
    ...
    | samtools sort -@ 8 -o aligned/sample_sorted.bam -

# Index the sorted BAM immediately after alignment
...
| Flag | Value | Meaning |
|---|---|---|
| `-t` | 8 | Number of CPU threads — use as many as your machine has |
| `-R` | `@RG\t...` | Read group string — required by GATK; see Section 5.5 |
| `2>` | logs/bwa.log | Redirect BWA-MEM2 progress messages to a log file |
| `-` | (pipe) | Read input from stdin (the piped BWA-MEM2 output) |

> 💡 **Why pipe directly to `samtools sort`?** A single human WGS SAM file can be 300–500 GB uncompressed. Piping the output of BWA-MEM2 directly into `samtools sort` sorts and compresses on the fly, writing only the final BAM to disk and saving both time and storage.
